...
| **بوت المخزون على تليجرام** | ✅ مكتمل ومُختبَر — الدالة نفسها شُغِّلت على Deno (`docs/telegram-bot.md`) |
...
- ✅ `tests/bot.test.sql` — 20 مجموعة لبوت المخزون: الحجز، التأكيد، الإلغاء، الصلاحيات، العدّاد
- ✅ `tests/local/bot-concurrency.test.sh` — 5 حالات تزامن: لا كود يخرج لبائعين، ولا تأكيدان لعملية
- ✅ `tests/local/bot-e2e.test.js` — 43 فحصاً على **دالة البوت نفسها وهي تعمل على Deno**، وتُقرأ الرسائل والأزرار التي أرسلتها فعلاً

**ما لم يُشغَّل** (يحتاج مشروعاً منشوراً): الـEdge Functions الستّ
الأخرى على Deno، رفع ملف حقيقي إلى Storage، وإرسال رسالة Telegram
حقيقية. منطق قاعدة البيانات الذي تستدعيه هذه الدوال مُختبَر بالكامل،
لكن الدوال نفسها لم تُنفَّذ قط — عدا `telegram-bot`، فهي الوحيدة التي
يشغّلها اختبارها فعلياً.
