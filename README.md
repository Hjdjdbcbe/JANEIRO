...
قائمة "نوع التفعيل" التي تظهر في صفحة المنتج تُقرأ من ملف واحد:
`frontend/activation-types.js`. عدّل النصوص فيه واحفظ — لا هجرة، ولا بناء،
ولا تعديل في مكان آخر. أول خيار في القائمة هو المختار افتراضياً، والحد
الأقصى 60 حرفاً للخيار الواحد.

ما يخزّنه الخادم هو النص الذي اختاره الزبون، ملتقَطاً مع سطر الطلب كما
يُلتقط اسم الخطة: تعديل القائمة لاحقاً لا يغيّر ما تقوله الطلبات القديمة.
والخادم يرفض أي سطر بلا اختيار (`MISSING_ACTIVATION_TYPE`) أو باختيار أطول
من 60 حرفاً — لكنه لا يحتفظ بنسخة من القائمة نفسها، حتى لا ترفض نسخةٌ
قديمة صياغتَك الجديدة.
